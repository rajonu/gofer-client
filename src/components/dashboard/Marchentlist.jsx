...
import Editmarchent from "./Editmarchent";
import { isAuth, getCookie } from "../../class/storage";
import { getMarchentList } from "../../class/getProducts";
            ...
            <div className='card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center'>
              ...
              <p className='mb-0 text-muted text-small w-15 w-xs-100'>
                <button
                  ...
                  {account_status === 1 ? "Inactive" : "Active"}
                </button>
              </p>
              <p className='mb-0 text-muted text-small w-15 w-xs-100'>
                <button
...
export const Marchentlist = () => {
  ...
  const handleModal = (id, name, address, payment_type, nid, phone) => {
    setValues({
      id,
      name,
      address,
  ...
  const removeMarchent = (id) => {
    $("#ok_button").click(async function () {
      try {
        let result = await axios({
          method: "DELETE",
          url: `${process.env.REACT_APP_API_URL}/removemarchent`,
          data: {
            id,
            token: getCookie("token"),
          },
        ...
        <div className='container-fluid disable-text-selection'>
          ...
          {finalMarchent.map((m, index) => (
            <DashboardDiv
              ...
              removeMarchent={removeMarchent}
            />
          ))}
          <Editmarchent {...values} setRefresh={setRefresh} />
          <Alert />
        </div>
      </main>
    </>
